...
SonetStream *sonet_stream_new(int fd, sonet_tls_t *tls) {
    SonetStream *s = calloc(1, sizeof(SonetStream));
    if (!s) return NULL;
    s->fd = fd;
    s->tls = tls;
    return s;
...
static void throttle(SonetStream *s, size_t bytes) {
    if (!s || s->limit_rate == 0 || bytes == 0) return;

    double now = sonet_utils_get_time_sec();
    ...
    if (s->bytes_this_sec >= s->limit_rate) {
        ...
        if (elapsed < 1.0) {
            sonet_sleep_ms((unsigned long)((1.0 - elapsed) * 1000));
        }
        s->last_throttle_time = sonet_utils_get_time_sec();
        s->bytes_this_sec = 0;
    }
}

static int fill_buffer(SonetStream *s) {
    if (s->read_buf.pos < s->read_buf.len) {
        return (int)(s->read_buf.len - s->read_buf.pos);
    }

    size_t to_read = SONET_STREAM_BUFFER_SIZE;
    if (s->limit_rate > 0) {
    ...
    int n;
    if (s->tls) {
        n = sonet_tls_read(s->tls, s->read_buf.data, (int)to_read);
    } else {
    ...
    if (n > 0) {
        ...
        if (err == WSAETIMEDOUT) return -SONET_ERR_TIMEOUT;
#else
        if (errno == ETIMEDOUT || errno == EAGAIN || errno == EWOULDBLOCK) return -SONET_ERR_TIMEOUT;
#endif
        return -SONET_ERR_NETWORK;
    } else {
        s->read_buf.pos = 0;
        s->read_buf.len = 0;
    }
...
int sonet_stream_read_exact(SonetStream *s, void *buf, size_t len) {
    ...
    while (total_read < len) {
        int n = sonet_stream_read(s, p + total_read, len - total_read);
        if (n <= 0) return n;
        total_read += n;
    }
...
int sonet_stream_write(SonetStream *s, const void *buf, size_t len) {
    ...
    int n;
    if (s->tls) {
        n = sonet_tls_write(s->tls, buf, (int)to_write);
    } else {
