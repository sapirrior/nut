...
SonetMultipart *sonet_multipart_new(void) {
    SonetMultipart *m = calloc(1, sizeof(SonetMultipart));
    if (!m) return NULL;
...
void sonet_multipart_add_file(SonetMultipart *m, const char *field_name, const char *filepath, const char *mime_type) {
...
const char *sonet_multipart_content_type(const SonetMultipart *m) {
    if (((SonetMultipart *)m)->cached_content_type) free(((SonetMultipart *)m)->cached_content_type);
    char buf[128];
    snprintf(buf, sizeof(buf), "multipart/form-data; boundary=%s", m->boundary);
    ((SonetMultipart *)m)->cached_content_type = strdup(buf);
...
void sonet_multipart_into_request(SonetMultipart *m, SonetRequest *req) {
    ...
    req->body_parts = calloc(max_parts, sizeof(SonetBodyPart));
    ...
    for (size_t i = 0; i < m->count; i++) {
        ...
        req->body_parts[req->body_parts_count].type = SONET_BODY_PART_MEM;
        ...
        if (p->is_file) {
            ...
            req->body_parts[req->body_parts_count].type = SONET_BODY_PART_FILE;
    ...
    sonet_headermap_set(req->headers, "Content-Type", sonet_multipart_content_type(m));
    return;

oom_cleanup:
    /* Free any already-allocated parts and signal failure */
    for (size_t j = 0; j < req->body_parts_count; j++) {
        if (req->body_parts[j].type == SONET_BODY_PART_MEM) {
            free((void *)req->body_parts[j].data);
        } else if (req->body_parts[j].type == SONET_BODY_PART_FILE) {
