...
sonet_cookie_jar_t *sonet_cookie_jar_create(void) {
    sonet_cookie_jar_t *jar = calloc(1, sizeof(sonet_cookie_jar_t));
    return jar;
}
...
sonet_cookie_jar_t *sonet_cookie_jar_load(const char *path) {
    FILE *f = fopen(path, "r");
    if (!f) {
        if (errno != ENOENT) {
            sonet_diag_err("could not open cookie jar '%s' for reading: %s", path, strerror(errno));
        }
        return NULL;
    }

    sonet_cookie_jar_t *jar = sonet_cookie_jar_create();
...
int sonet_cookie_jar_save(const sonet_cookie_jar_t *jar, const char *path) {
    if (!jar) return -1;
    FILE *f = fopen(path, "w");
    if (!f) {
        sonet_diag_err("could not open cookie jar '%s' for writing: %s", path, strerror(errno));
        return -1;
    }

    fprintf(f, "# Netscape HTTP Cookie File\n");
    for (size_t i = 0; i < jar->count; i++) {
        sonet_cookie_t *c = &jar->cookies[i];
...
void sonet_cookie_jar_add(sonet_cookie_jar_t *jar, const sonet_cookie_t *cookie) {
    ...
    sonet_cookie_t *temp = realloc(jar->cookies, sizeof(sonet_cookie_t) * (jar->count + 1));
    if (temp) {
        jar->cookies = temp;
